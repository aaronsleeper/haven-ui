...
| Two-Pane Layout | _(no PL page — utility classes)_ | `layout-two-pane`, `layout-two-pane-grid` | no | Bounded page width (1400px) + a rail/main two-column grid. `layout-two-pane` on the page container; `layout-two-pane-grid` on the grid wrapper — 420px rail + fluid main at `lg`, single column below. Promoted 2026-05-16 from inline arbitrary-value utilities used across the cena-uconn handoff. **Consumer-pairing rule (HVD-01, 2026-06-04):** the grid gives both panes the same top edge; consumers carry their own top-padding equalization when paired with content-flow surfaces (chat-thread, document-shell) that have inner top padding. The grid stays consumer-agnostic. |
| Header Pair | `header-pair.html` | `header-pair`, `header-pair-primary`, `header-pair-secondary` | no | Primary identifier + secondary state/status label composition. Stacks vertically at <sm; horizontal with space-between at >=sm. Used by any record header that pairs primary (Lora identifier register) + secondary (uppercase tracking-wider state register). Consumers: `.patient-recall-list-header`. UX-06 canon, 2026-06-04. |
...
| Escalation Callout | `doc-escalation-callout.html` | *(composition)* `alert-error`, `alert-stripe`, `screen-ref` | no | Trigger → who → how contact protocol for a specific escalation (e.g. cap-27 safety concern). Pure composition — an `alert-stripe` in the error register + bold trigger line + labeled Who/How. Distinct from the decision-branch severity flags: flags say *when* a situation crosses a threshold, the callout gives the actionable contact protocol. |
| Screen Reference | `doc-screen-reference.html` | `screen-ref`, `screen-ref-icon`, `screen-ref-sep` | no | Wayfinding chip — which interface + where, linked to the owning capability. An `<a>` sand chip (display icon + chevron-separated path), inline in a procedure step's "where" line or standalone. A quieter, bordered register distinct from `text-link`'s action affordance — this is wayfinding, not a commit. |
| Diagram Figure | `doc-diagram-figure.html` | `diagram-figure`, `diagram-figure-caption`, `diagram-figure-eyebrow`, `diagram-figure-title`, `diagram-figure-asset`, `diagram-figure-description` | no | Resolved-asset figure embedding a workflow swim-lane diagram inline in a document body. Authored via `:::diagram{workflow="<slug>"}` directive; runner resolves slug → use-case dir → spec_to_haven_svg standalone (HTML iframe srcdoc) + Chrome-headless PNG (DOCX inline image). Brand spec §3.13. First resolved-asset directive in the haven vocabulary — the figure-frame layer is the PL contract; the asset itself (renderer-internal `diagram-*` classes) migrates to components.css as a future Tier 1 PL slice. Restraint cap: ≤2 diagrams per SoP (HVD Q5 soft cap). |
| Escalation | `doc-escalation.html` | `escalation` | no | Routing rule with safety weight — "when something already broke, do X." Single haven primitive matching brand spec §3.10: amber-700 left rule on sand-50 warm ground. Distinct from the older Escalation Callout composition (`.alert .alert-error .alert-stripe`) which remains valid for app-level use where the icon-column + red-error register is wanted; this primitive is what the `:::escalation` directive renders to. Brand spec is canon; reconciling the older composition to the same primitive is a tracked follow-up. |
| Glossary Pair | `doc-glossary-pair.html` | `glossary-term`, `glossary-def` | no | Sequential term/definition pairs authored directly in document flow via `:::glossary-term` + `:::glossary-def` directives. Bolded term with tight space-after; indented definition with looser space-after. Brand spec §3.12. Sibling to (not replacement of) the Block Glossary composition (`.card` + `.kv-table`); use Glossary Pair when directive vocabulary produces sequential top-level pairs without table grouping. |
| Review Marker | `doc-review-marker.html` | `review-marker` | no | Visible scaffolding for unresolved review prompts in pre-approval SoP drafts. Dashed amber-700 left rule on amber-50 bg distinguishes "the team is asking the reviewer this" from "the procedure says this." Pairs with `:::review-marker` directive. Strip every marker before the SoP ships to operational use; the chrome makes leftover markers obvious. Brand spec §3.14. UX-panel source incident: 2026-06-07 Care Coordinator SoP HTML review found `[NEEDS / CONFIRM]` markers rendering as inline `<code>` inside binding clinical instructions. |
...
| Hue Family Picker | `forms-hue-family-picker.html` | `hue-family-picker`, `hue-family-picker-header`, `hue-family-picker-family-label`, `hue-family-picker-family-select`, `hue-family-picker-swatch`, `hue-family-picker-stop-readout`, `hue-family-picker-stop-label`, `hue-family-picker-stop-role`, `hue-family-picker-gamut-badge`, `hue-family-picker-gamut-badge-text`, `hue-family-picker-slider-wrap`, `hue-family-picker-track`, `hue-family-picker-ticks`, `hue-family-picker-tick`, `hue-family-picker-tick-label`, `hue-family-picker-values`, `hue-family-picker-value-row`, `hue-family-picker-value-label`, `hue-family-picker-value`, `hue-family-picker-copy-btn`, `hue-family-picker-expand-toggle`, `hue-family-picker-drawer` (+ shared `slider-track` base; modifiers: `is-compact`, `is-expanded`) | no | Single slider along a Haven hue-family OKLCH line. Continuous interpolation between `min`/`root`/`max` anchors with 11 canonical stops as snap-targets at every 10%. Displays OKLCH/HEX/RGB/HSL with copy-to-clipboard. JS: `hue-family-picker.js` (ES module — imports culori). `data-hue-family-picker` to init; `data-family` + `data-value` for initial state. Promotes shared `.slider-track` base extracted from `assess-slider`. **Compact variant** (`.is-compact`): dense single-row layout for stacked use in theming editors — HEX inline, OKLCH/RGB/HSL behind chevron drawer (`.is-expanded`); target ~56–64px per row. |
...
| Patient Recall List | `patient-recall-list.html` | `patient-recall-list`, `patient-recall-list-header`, `patient-recall-list-header-day`, `patient-recall-list-header-pass`, `patient-recall-list-rows`, `patient-recall-list-row`, `patient-recall-list-row-name`, `patient-recall-list-row-when`, `patient-recall-list-row-amount`, `patient-recall-list-row-trash`, `patient-recall-list-footer`, `~~patient-recall-list-add~~` (deprecated), `patient-recall-list.is-locked` | no | Multi-pass dietary recall display in the right-pane during chat-driven recall flow. Header (now composes `.header-pair` — UX-06; day in Lora Medium 16 + pass indicator stack at <sm) + 4-col grid rows (name + when + amount + trash). Columns populate progressively across passes 1–4; passes 5–6 lock the list (`.is-locked` hides trash + footer). Auto-gets `lg:pt-6` for HVD-01 consumer-pairing alignment. `.patient-recall-list-add` "+ add" footer **deprecated 2026-06-04** (UX-03 + HVD-03) — agent's verbal continuation carries discoverability; slice-1 retrofit removes the DOM. Renamed from food-recall-list. Covers gap-doc R-21. |
| Patient Recall (Mobile Composition) | `patient-recall-mobile.html` | `patient-recall-mobile-trigger`, `patient-recall-desktop-pane` | yes — hs-overlay | Mobile-canonical composition entry. Renders agent-uttered `chat-sheet-link` → `bottom-sheet-panel` containing `patient-recall-list` at <lg; renders `patient-recall-list` directly in the right-pane via `layout-two-pane-grid` at >=lg. One DOM, viewport-switched. Pull-to-reveal explicitly rejected (conflicts with `bottom-sheet-handle` drag-to-dismiss). UX-02 canon, 2026-06-04. |
