...
| MealOptionCard | `(agentic)` | `meal-option-card`, `meal-option-card-image`, `meal-option-card-placeholder`, `meal-option-card-content`, `meal-option-card-name`, `meal-option-card-description`, `meal-option-card-footer`, `meal-option-card-price`, `meal-option-card-tags` | no | Browse + select meal item for chat-pane meal-ordering: image + name + description + price + tags + action. Distinct from MealDeliveryCard (idle-pane delivery + swap). |
...
| MenuGrid | `(agentic)` | `menu-grid`, `menu-grid-header`, `menu-grid-title`, `menu-grid-subtitle` | no | Grid container for MealOptionCard collection. |
| BasketReview | `(agentic)` | `basket-items`, `basket-item`, `basket-item-name`, `basket-item-qty`, `basket-item-price`, `basket-totals`, `basket-total-row`, `basket-budget-remaining` | no | Order basket with line items, totals, budget remaining. |
...
| Quantity Stepper | `quantity-stepper.html` | `quantity-stepper`, `quantity-stepper-btn`, `quantity-stepper-value` | no | Patient-app +/- stepper for adjusting numeric quantities. Pill-shaped row with secondary-100 surface; minus + tabular-numeric value cell + plus. Native `disabled` attribute on at-min/at-max buttons. Distinct from kitchen-meal-assignment-grid's `meal-qty-cell` (kitchen-only tabular pattern). |
...
| Patient Week Panel | `patient-week-panel.html` | `patient-week-panel`, `patient-week-panel-section`, `patient-week-panel-section-label` (composes `budget-meter`, `patient-delivery-status-card`, `appointment-card`) | no | Tier 2 idle-state right-pane composition stacking 3 existing primitives: budget-meter (top, primary read), patient-delivery-status-card (middle, time-bounded), appointment-card (bottom, longest-horizon). No new visual primitives — just a layout wrapper that establishes consistent vertical rhythm + optional eyebrow labels per section. Renamed from at-a-glance-panel. Covers gap-doc R-30. |
...
| Option Row | `option-row.html` | `option-row`, `option-row-glyph`, `option-row-content`, `option-row-title`, `option-row-recommended`, `option-row-description`, `option-row-check`, `option-row.is-other`, `option-row-other-wrapper`, `option-row-other-textarea`, `option-row.is-tablet-dense`, `option-row-list` | no | Tier 1 primitive for the agentic-question pattern. `<button role="radio">` (single-select) or `<button role="checkbox">` (multi-select) + `aria-checked`. Title + description + inline `.badge.badge-sm.option-row-recommended` slot + check icon. Triple-cued selection (filled glyph + check icon + accent-interactive border). `.is-other` reveals an inline textarea on select (gov.uk "Conditionally revealed questions" pattern); `aria-expanded` + `aria-controls` link button to textarea. `.is-tablet-dense` raises `--option-row-min-height` from 44px to 48px for kitchen gloved-hand context. Roving tabindex is consumer-side state. NOT response-option (assessment-only per steward verdict 2026-05-08). |
