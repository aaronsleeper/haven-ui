...
| Color Picker | `forms-color-picker.html` | `color-picker`, `color-picker-label`, `color-picker-swatches`, `color-picker-swatch`, `color-picker-swatch-sm`, `color-picker-swatch-custom`, `color-picker-preview`, `color-picker-preview-dot`, `color-picker-value`, `color-picker-disabled` | no | Swatch selector for meal type / status. JS: `color-picker.js`. `data-color-picker` to init. Small variant with `-sm`. Custom swatch bridges native `input[type=color]`. |
