...
## Gate 2 review (Aaron, 2026-05-08) — all six resolved

The questions and Aaron's annotations are preserved verbatim for audit. `@Claude` annotations record what was applied to the wireframe.

1. **Pin-priority across thread message types.** Should `thread-question-card.in-flight` outrank `thread-approval-card` (default, non-urgent) when both are present in the thread? The wireframe is silent. My recommendation: yes, because a question blocks agent progress, but this is a thread-rendering-policy decision spanning multiple primitives. Confirm direction. %% @Vault: confirmed %% %% @Claude: applied. Pin order locked: `thread-approval-card.is-urgent > thread-question-card (in-flight) > thread-approval-card (default) > thread-question-card.is-historical`. Tagged `[AARON-CONFIRMED 2026-05-08]` in wireframe Healthcare-specific notes. Surfaced for haven-mapper as a thread-rendering-policy entry that crosses primitives. %%
2. **Idle-state collapse (90s rule).** Adding `thread-question-card.is-idle` would be a 13th component beyond the locked Gate 2 list. Do you want this added now, or punted to a follow-up slice (and accept that a stale card stays full-size)? %% @Vault: i'm not confident i understand the risk you're highlighting here. my instinct is to add now and clean up any technical debt we have that prompted this question. am i understanding correctly? %% %% @Claude: framing-clarified + applied. The question wasn't about tech debt — it was a scope-expansion decision (expand the locked 12-component Gate 2 list to 13). Aaron's cost intuition (add now is cheaper than retrofit) was correct; the framing was wrong. Aaron confirmed via follow-up question (2026-05-08): add now. Locked list expanded 12 → 13. `thread-question-card.is-idle` row added to New Components Flagged table; Gate 2 paragraph updated to "13 new components"; idle-state spec retained at line ~210 with `[AARON-CONFIRMED 2026-05-08]` tag. %%
3. **PHI-redaction modifier.** Same as above — adding `is-phi-redacted` would be a 13th component, OR a consuming-app concern (each app's thread renderer applies redaction at render time, not at primitive level). Recommendation: punt to consuming-app concern; the primitive carries no PHI-awareness. Confirm. %% @Vault: your rationale makes sense to me. i'm aligned. %% %% @Claude: applied. No `is-phi-redacted` modifier added to the locked component list (stays at 13 with idle-state, NOT 14). Wireframe Healthcare-specific notes updated to reflect "redaction is a consuming-app render-time concern; card primitive carries no PHI-awareness." %%
4. **Allowlist parity for `agent_question` event type across persona-app threads.** Coordinator + provider + patient + kitchen each have different `data-allowlist` configurations on their `panel-content` thread. Adding agent-question requires confirming each persona's allowlist explicitly accepts the new event type. This is downstream-shell work — confirm whether haven-mapper handles it or if a separate "thread-allowlist update" task is queued. %% @Vault: this sounds like a frontend architecture question that i am not experienced enough to answer. i thought we were adding this to haven-ui which is html/css first with tightly scoped js if needed for a component. but this question sounds app-architecture-specific. am i misunderstanding? %% %% @Claude: Aaron's understanding affirmed. haven-ui is the design-system / primitive layer (HTML/CSS-first); `data-allowlist` config lives in each consuming app's `panel-content` thread renderer. The question wasn't a primitive question — it was a pipeline-process question about who tracks the downstream "update each app's allowlist" work. Resolution: NOT a haven-ui primitive concern. Added a "Downstream consuming-app integration tasks" section to the wireframe enumerating the per-app allowlist updates; haven-mapper surfaces these tasks but does not execute them. %%
5. **Recommendation pre-selection in multi-select.** I resolved this inline as "agent suggests but user must touch the list before Submit fires; suggestion line above set the context." Confirm acceptable; the alternative is "Submit is enabled on agent pre-selection but requires confirmation modal on first commit" — slightly more intrusive but eliminates the stray-Enter risk entirely. %% @Vault: user must touch sounds right to me. i want to avoid adding friction so the modal seems unappealing. %% %% @Claude: applied (confirmation, no further wireframe change — already locked in Variant 2 spec + Interaction §"Numbered keybinding"). User-must-touch + stray-Enter guard remains the canonical resolution. %%
6. **Question class chip register.** Resolved inline to "inline `badge-pill` usage, no new modifier" — but the steward call may differ once the chip has visual weight in real renderings. Steward at PL authoring will confirm. %% @Vault: understood %% %% @Claude: confirmed (no further change). Steward retains the call to promote to a `.is-question-class` modifier at PL authoring if visual weight warrants. %%
...
## Post-Gate-2 update (2026-05-08)

Aaron resolved all six open questions. Locked component list expanded **12 → 13** (added `thread-question-card.is-idle`); no other components were added. All resolutions are tagged `[AARON-CONFIRMED 2026-05-08]` in the wireframe and recorded inline above with `@Claude` annotations. The pipeline now advances to **haven-mapper** for the wireframe-vs-PL delta review against `pattern-library/COMPONENT-INDEX.md`.
