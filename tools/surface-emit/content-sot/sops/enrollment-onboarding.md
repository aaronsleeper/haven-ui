...
Work the steps in order. The first four are the care coordinator's; the last two are the dietitian's.
...
2. **Care Coordinator — Obtain IRB consent.** In Athena, using the UConn-provided consent form (see the consent paths below). Result: consent is recorded on the participant record with a timestamp, before anything else proceeds. _Note: no participant takes part in the program until consent is captured. Athena is the system of record for consent._
...
4. **Care Coordinator — Complete the intake assessment (within 7 days).** In the intake form on the participant record. Result: demographics, medical history, dietary assessment, and cultural food preferences are captured. _Note: some fields import from Athena or the referral; the rest come from talking with the participant._

5. **Registered Dietitian — Administer the baseline assessments (Month 0).** In the Cena assessment forms on the participant record. Result: HFIAS, HEI (via 24-hour dietary recall), WHOQOL-HIV BREF, and NKQ are completed and scored. _Note: these are the study's baseline — they repeat at later months, so accuracy now matters for the whole program._

6. **Registered Dietitian — Build the initial care plan.** In [Cena platform → Care plan]{.screen-ref}. Result: an individualized nutrition care plan and goals, seeded from intake and the baseline assessments. The participant is now enrolled.

## How consent arrives

Consent reaches you one of three ways. Confirm which, then proceed.

::: decision-branch
**Referral path** — Athena may already carry the consent from UConn; confirm it's on the record before proceeding.

**Non-referral path** — Create the record in Athena and obtain consent directly from the participant.

**Paper backup** — Consent signed on paper in clinic; enter it into Athena so the record is complete.
:::
...
- [ ] Capture IRB consent before anything else
- [ ] Create or confirm the participant account in Athena
- [ ] Complete the intake assessment within 7 days
- [ ] Administer the Month 0 baseline assessments (RD)
- [ ] Build the initial care plan (RD)
...
:::
::: glossary-def
The participant's agreement to take part in the study, on UConn's form. Required before anything else.
:::
...
:::
::: glossary-def
Four standard questionnaires given at Month 0 (and again later) to measure the program's effect over time.
:::
...
:::
::: glossary-def
The participant's individual nutrition plan and goals, built from intake and the baseline assessments.
:::
