...
- **Version — read 2026-05-25: Angular 21.2.13** (`patients/package.json`, `@angular/core: ^21.2.13`). Resolves the earlier guesses (the idioms doc said "19"; this doc had inferred "≥19.2, likely 20" from the stable `provideZonelessChangeDetection()` API + monorepo siblings — both undershot). His use of stable zoneless + `linkedSignal` is consistent with 21. The porter pins to **21.x** API names; one built against Angular-20 names would emit wrong (zoneless/`linkedSignal`/control-flow surface). `config`-cost row #16 in the ledger. **CONFIRMED — no longer low-confidence.**
